    ...
    return (
        <div className="relative my-4 rounded border-2 border-slate-300 transition-all duration-300 focus-within:border-blue-700">
            <input
                ...
                required
                className="peer w-full p-3 text-xl outline-none"
                onChange={onChange}
                onInvalid={onInvalid}
            />
            <label
                htmlFor={id}
                className="absolute left-1 top-0 bottom-0 my-auto block h-fit cursor-text bg-white px-1 text-xl text-slate-400 transition-all duration-500 peer-valid:-translate-y-full peer-valid:text-lg peer-focus:-translate-y-full peer-focus:text-lg peer-focus:text-blue-700"
            >
                {label}
            </label>
        </div>
